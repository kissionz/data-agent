...
| 浏览器自动 E2E | 部分覆盖，`tests/e2e/prd-acceptance.desktop.spec.ts` 和 `tests/e2e/prd-acceptance.mobile.spec.ts` 使用 Playwright + 本机 Chrome 覆盖标准查询结果、表格替代、口径证据、澄清、权限拒绝、语义指标评审/认证、数据源降级质量门禁、受限字段样本策略、协作资产重新鉴权/水印/订阅阻断、运营回放详情和移动端会话/上下文面板；仍需扩展取消、部分结果、真实导出文件、跨刷新恢复和 WebKit/Safari。 |
...
4. 继续扩展可重复浏览器测试：取消、部分结果、真实导出文件、跨刷新恢复和 WebKit/Safari。
