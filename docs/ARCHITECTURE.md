...
| E2E | `tests/e2e/*` | Playwright + 本机 Chrome 覆盖标准查询、表格替代、口径证据、澄清、权限拒绝、语义指标评审/认证、数据源降级质量门禁、受限字段样本策略、协作资产重新鉴权/水印/订阅阻断、运营回放和移动端面板可达 | 扩展 WebKit/Safari、取消、部分结果、真实导出文件和跨刷新恢复 |
