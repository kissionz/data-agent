...
export const trendResult: RunResult = {
  ...
  chartSpec: {
    id: 'chart_net_revenue_trend',
    title: '净收入月度趋势',
    description: '按月展示认证指标净收入，时间轴升序，纵轴使用万元展示。',
    type: 'line',
    xAxisColumnId: 'month',
...
export const emptyResult: RunResult = {
  ...
  rows: [],
  chartSpec: {
    id: 'chart_empty_region',
    title: '当前条件无可图表化数据',
    description: '空结果必须展示为空状态或数据表，避免绘制误导性图表。',
    type: 'table',
    yAxisColumnIds: [],
    source: 'validated_result_spec',
    safety: { grounded: true, warnings: ['Empty result should be presented as a table or empty state, not a chart.'] },
  },
  completeness: 'full',
  incompleteSteps: [],
