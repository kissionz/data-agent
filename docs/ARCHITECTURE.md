...
Query Gateway 是 F06 的安全边界：Compiler 只接受规范 Analysis IR 和 Semantic Catalog 裁决后的 metric/dimension ID，绑定受治理的 `dataSourceId` 与输出 schema，生成带 tenant/workspace/business domain 守卫的只读参数化 SQL。Node-only PostgreSQL adapter 在同一个 `BEGIN READ ONLY` 事务内执行 JSON EXPLAIN、真实预算判定和查询正文，设置 statement timeout，并通过独立池连接调用 `pg_cancel_backend` 响应 worker 的 AbortSignal。PostgreSQL API 模式提交后返回 202/querying，内存 Run queue 使用 attempt、lease token、fence、重试和幂等取消防止旧 worker 发布；真实 rows 经 schema 校验和 grounded result mapper 后才进入 completed Run。`PublicRunView` 仍只返回 public-safe 指纹、预算、缓存与取消摘要，不暴露 SQL、参数、连接串或数据库对象细节。下一步需把内存 queue、Run CAS、result pages 和 events 落到同一 PostgreSQL 完成事务，才能支持独立 API/worker 多进程和大结果原子发布。
