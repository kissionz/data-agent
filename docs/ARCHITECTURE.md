...
| Persistence | `src/persistence/*` | conversation、run、idempotency、audit events 端口、内存 adapter、本地 JSON 文件 adapter、SQL migration、可替换 SQL adapter、retention cleanup planner；SQL 审计事件单独落表；默认留存为问题/IR/SQL 指纹 180 天、结果摘要 30 天、原始结果 7 天、敏感样本 3 天、审计 365 天 | 接具体 SQLite/PostgreSQL/Redis driver、migration runner、连接池、生产审计查询和实际清理调度 |
