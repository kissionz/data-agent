...
`docker-compose.postgres.yml` 会先按生产顺序装载纯 control-plane 001–006，再单独装载编号 900 的集成测试数据和 `chatbi_reader` 角色。生产部署只运行 `npm run migrate:control-plane`；`scripts/postgres/init.sql` 含本地 fixture、测试角色和测试数据库授权，禁止作为生产迁移执行。
