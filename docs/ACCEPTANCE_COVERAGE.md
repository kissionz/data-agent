...
| 契约测试 | 已覆盖，`src/test/contracts.test.ts` 验证未知字段拒绝、IR schema 与安全护栏；`src/test/contractsPackage.test.ts` 验证 `@insightflow/contracts` 包入口和 `api`/`domain`/`events` 子路径导出版本、schema、错误码、公共状态词表和 SSE helper。 |
...
1. 基于 `@insightflow/contracts` 的 schema 生成生产 OpenAPI 和 SDK，减少手写 `openApiDocument`。
